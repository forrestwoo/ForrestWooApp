...
-(UIImage *) imageCompressForWidth:(UIImage *)sourceImage targetHeight:(CGFloat)defineHeight{
    ...
    [sourceImage drawInRect:thumbnailRect];
    NSLog(@"...%@...",NSStringFromCGRect(thumbnailRect));
    newImage = UIGraphicsGetImageFromCurrentImageContext();
    if(newImage == nil){
        NSLog(@"scale image fail");
    }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    UIImage *image = [self imageCompressForWidth:selectedImage targetWidth:375];
    if (image.size.height > 520) {
        image = [self imageCompressForWidth:selectedImage targetHeight:520];
    }
    currentImage = image;
